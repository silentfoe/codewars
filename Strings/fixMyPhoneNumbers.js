...
function isItANum(str) {
  ...
  const number = str.split('').filter(elem => !isNaN(elem)).join('').replaceAll(' ', '')
   
  return number[0] === '0' && number.length === 11 ? number : "Not a phone number"
  
}

// second solution not using replaceAll and filtering through all elements the first time around

function isItANum(str) {

  const number = str.split('').filter((elem,indx) => str.charCodeAt(indx) > 47 && str.charCodeAt(indx) < 58).join('')
